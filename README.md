...
Root Signal sits between fragmented community sources and people who want to act. The Scout agent crawls curated URLs, web search results, RSS feeds, and social media accounts for a city, then extracts structured signals using LLMs. Signals are deduplicated across three layers (exact match, URL-scoped, vector similarity), clustered into stories, and persisted into a knowledge graph. A supervisor monitors signal health, source quality, and coverage gaps.
