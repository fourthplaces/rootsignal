...
impl ConnectionInfo {
    pub(crate) fn new(
        ...
        let (routing, encryption, skip_verify) = match url.scheme() {
            "bolt" | "" => (false, false, false),
            "bolt+s" => (false, true, false),
            "bolt+ssc" => (false, true, true),
            "neo4j" => (true, false, false),
            "neo4j+s" => (true, true, false),
            "neo4j+ssc" => (true, true, true),
            otherwise => return Err(Error::UnsupportedScheme(otherwise.to_owned())),
        };

        let encryption = if encryption {
            Some(Self::tls_connector(url.host(), client_certificate, skip_verify)?)
        } else {
            None
        };
    ...
    fn tls_connector(
        ...
        let config = if skip_verify {
            // bolt+ssc: accept any certificate (including self-signed CA certs)
...
/// Used for bolt+ssc:// connections (self-signed certs, including those with CA:TRUE).
#[derive(Debug)]
