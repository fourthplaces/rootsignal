...
impl From<taproot_domains::search::SearchResultRow> for GqlSearchResult {
    fn from(r: taproot_domains::search::SearchResultRow) -> Self {
        Self {
            id: r.id,
            title: r.title,
...
impl GqlSearchResult {
    async fn entity(&self, ctx: &Context<'_>) -> Result<Option<GqlEntity>> {
        // Look up entity_id from the listings table
        let pool = ctx.data_unchecked::<sqlx::PgPool>();
        let row = sqlx::query_as::<_, (Option<Uuid>,)>(
            "SELECT entity_id FROM listings WHERE id = $1",
        )
        .bind(self.id)
        .fetch_optional(pool)
        .await
        ...
        let entity_id = match row.and_then(|r| r.0) {
            Some(id) => id,
            None => return Ok(None),
        };
...
impl From<taproot_domains::search::SearchResponse> for GqlSearchResponse {
    fn from(r: taproot_domains::search::SearchResponse) -> Self {
        Self {
...
impl From<taproot_domains::search::ParsedQuery> for GqlParsedQuery {
    fn from(p: taproot_domains::search::ParsedQuery) -> Self {
        Self {
...
impl From<taproot_domains::search::ParsedFilters> for GqlParsedFilters {
    fn from(f: taproot_domains::search::ParsedFilters) -> Self {
        Self {
...
impl From<taproot_domains::search::ParsedTemporal> for GqlParsedTemporal {
    fn from(t: taproot_domains::search::ParsedTemporal) -> Self {
        Self {
...
impl From<taproot_domains::search::SearchIntent> for GqlSearchIntent {
    fn from(i: taproot_domains::search::SearchIntent) -> Self {
        match i {
            taproot_domains::search::SearchIntent::InScope => Self::InScope,
            taproot_domains::search::SearchIntent::OutOfScope => Self::OutOfScope,
            taproot_domains::search::SearchIntent::NeedsClarification => Self::NeedsClarification,
            taproot_domains::search::SearchIntent::KnowledgeQuestion => Self::KnowledgeQuestion,
        }
    }
}
