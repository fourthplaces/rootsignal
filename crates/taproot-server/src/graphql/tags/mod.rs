...
impl TagQuery {
    ...
    async fn tags(&self, ctx: &Context<'_>, kind: Option<String>) -> Result<Vec<GqlTag>> {
        ...
        let tags = if let Some(kind) = kind {
            taproot_domains::entities::Tag::find_by_kind(&kind, pool)
                .await
                .unwrap_or_default()
        } else {
            sqlx::query_as::<_, taproot_domains::entities::Tag>(
    ...
    async fn tag_kinds(&self, ctx: &Context<'_>) -> Result<Vec<GqlTagKind>> {
        let pool = ctx.data_unchecked::<sqlx::PgPool>();
        let kinds = taproot_domains::entities::TagKindConfig::find_all(pool)
            .await
            .unwrap_or_default();
