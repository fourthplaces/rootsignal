...
async fn hybrid_ranked_search(
    ...
    qb.push(
        "LEFT JOIN translations t_title ON t_title.translatable_type = 'listing' \
         AND t_title.translatable_id = l.id AND t_title.field_name = 'title' AND t_title.locale = ",
    );
    qb.push_bind(params.locale.clone());
    qb.push(" ");
    qb.push(
        "LEFT JOIN translations t_desc ON t_desc.translatable_type = 'listing' \
         AND t_desc.translatable_id = l.id AND t_desc.field_name = 'description' AND t_desc.locale = ",
    );
    qb.push_bind(params.locale.clone());
    qb.push(" ");
    qb.push(
        "LEFT JOIN translations en_title ON en_title.translatable_type = 'listing' \
         AND en_title.translatable_id = l.id AND en_title.field_name = 'title' AND en_title.locale = 'en' ",
    );
    qb.push(
        "LEFT JOIN translations en_desc ON en_desc.translatable_type = 'listing' \
         AND en_desc.translatable_id = l.id AND en_desc.field_name = 'description' AND en_desc.locale = 'en' ",
    );
        ...
        if let Some(val) = value {
            qb.push(
                "AND EXISTS (SELECT 1 FROM taggables tg JOIN tags t ON t.id = tg.tag_id \
                 WHERE tg.taggable_type = 'listing' AND tg.taggable_id = l.id AND t.kind = ",
            );
            qb.push_bind(*kind);
            qb.push(" AND t.value = ");
