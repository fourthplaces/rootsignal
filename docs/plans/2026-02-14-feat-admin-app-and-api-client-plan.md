...
Add two new JS/TS packages to the taproot monorepo: `api-client-js` (typed GraphQL client via codegen + fetch) and `admin-app` (Next.js). The admin app provides CRUD, moderation, and operational tooling for the taproot platform. Auth uses Twilio Verify OTP → JWT → HTTP-only cookies, matching the proven mntogether pattern.
...
Taproot currently has no admin interface. All data management happens through direct database access or the Restate workflow pipeline. Operators need a web UI to:
...
```
┌─────────────────┐     ┌──────────────────────┐     ┌─────────────┐
│   admin-app     │────▶│   taproot-server      │────▶│  PostgreSQL  │
...
**1e. Auth domain in taproot-server**
...
- Update `Observation` model in `taproot-domains`
