...
impl ActorService for ActorServiceImpl {
    ...
    async fn create_manual(
        ...
        Ok(ctx.run(|| async move {
            let result: anyhow::Result<CreateManualActorResult> = async {
                let writer = GraphWriter::new(deps.graph_client.clone());
            ...
            .await;
            result.map_err(|e| -> HandlerError { TerminalError::new(e.to_string()).into() })
        })
        .await?)
    }
    ...
    async fn add_account(
        ...
        Ok(ctx.run(|| async move {
            let result: anyhow::Result<AddAccountResult> = async {
                let writer = GraphWriter::new(deps.graph_client.clone());
