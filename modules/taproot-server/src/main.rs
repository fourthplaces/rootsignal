...
#[derive(Parser)]
#[command(name = "taproot-server", about = "Taproot community signal server")]
struct Cli {
...
async fn main() -> Result<()> {
    ...
    let file_config = taproot_core::file_config::load_config(&config_path)?;
    let toml_value = taproot_core::file_config::load_toml_value(&config_path)?;

    // Load and resolve prompt templates
    let prompts = taproot_core::PromptRegistry::load(&file_config, config_dir, &toml_value)?;
    ...
    let http_deps = Arc::new(taproot_core::ServerDeps::new(
        http_pool.clone(),
        http_client.clone(),
        openai.clone(),
        claude.clone(),
    ...
    let worker_deps = Arc::new(taproot_core::ServerDeps::new(
        worker_pool,
        http_client,
        openai,
        claude,
    ...
    let restate_endpoint = restate_sdk::endpoint::Endpoint::builder()
        .bind(
            taproot_domains::scraping::ScrapeWorkflowImpl::with_deps(worker_deps.clone()).serve(),
        )
        .bind(
            taproot_domains::scraping::SourceObjectImpl::with_deps(worker_deps.clone()).serve(),
        )
        .bind(
            taproot_domains::scraping::SchedulerServiceImpl::with_deps(worker_deps.clone())
                .serve(),
        )
        .bind(
            taproot_domains::extraction::ExtractWorkflowImpl::with_deps(worker_deps.clone())
                .serve(),
        )
        .bind(
        ...
        .bind(
            taproot_domains::translation::TranslateWorkflowImpl::with_deps(worker_deps.clone())
                .serve(),
        )
        .bind(
            taproot_domains::listings::ListingsServiceImpl::with_deps(worker_deps.clone())
                .serve(),
        )
        .bind(
            taproot_domains::entities::TagsServiceImpl::with_deps(worker_deps.clone())
                .serve(),
        )
        .bind(
            taproot_domains::clustering::ClusteringJobImpl::with_deps(worker_deps.clone())
                .serve(),
        )
        .build();
