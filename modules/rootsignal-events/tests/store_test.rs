...
async fn child_event_sets_parent_and_caused_by() {
    ...
    let grandchild = child
        .append(AppendEvent::new("grandchild_event", json!({"step": "grandchild"})))
        .await
        .unwrap();
...
async fn read_from_returns_events_in_order() {
    ...
    store.append(AppendEvent::new("event_a", json!({"n": 1}))).await.unwrap();
    store.append(AppendEvent::new("event_b", json!({"n": 2}))).await.unwrap();
    store.append(AppendEvent::new("event_c", json!({"n": 3}))).await.unwrap();
...
async fn read_tree_returns_full_causal_chain() {
    ...
    let root = store
        .append(AppendEvent::new("scrape", json!({"url": "https://example.com"})))
        .await
        .unwrap();
    ...
    extract
        .append(AppendEvent::new("signal_discovered", json!({"title": "Cleanup"})))
        .await
        .unwrap();

    extract
        .append(AppendEvent::new("signal_discovered", json!({"title": "Food Drive"})))
        .await
        .unwrap();
...
async fn read_by_type_filters_correctly() {
    ...
    store.append(AppendEvent::new("signal_discovered", json!({}))).await.unwrap();
    store.append(AppendEvent::new("url_scraped", json!({}))).await.unwrap();
    store.append(AppendEvent::new("signal_discovered", json!({}))).await.unwrap();
    store.append(AppendEvent::new("citation_recorded", json!({}))).await.unwrap();

    let signals = store.read_by_type("signal_discovered", 1, 100).await.unwrap();
...
async fn read_by_run_returns_run_events() {
    ...
    store.append(AppendEvent::new("event_a", json!({})).with_run_id("run-1")).await.unwrap();
    store.append(AppendEvent::new("event_b", json!({})).with_run_id("run-2")).await.unwrap();
    store.append(AppendEvent::new("event_c", json!({})).with_run_id("run-1")).await.unwrap();
...
async fn latest_seq_returns_max() {
    ...
    store.append(AppendEvent::new("a", json!({}))).await.unwrap();
    store.append(AppendEvent::new("b", json!({}))).await.unwrap();
    let last = store.append(AppendEvent::new("c", json!({}))).await.unwrap();
...
async fn read_from_respects_limit() {
    ...
    for i in 0..10 {
        store.append(AppendEvent::new("event", json!({"i": i}))).await.unwrap();
    }
...
async fn read_from_gap_free_stops_at_gap() {
    ...
    store.append(AppendEvent::new("a", json!({}))).await.unwrap();
    store.append(AppendEvent::new("b", json!({}))).await.unwrap();
    store.append(AppendEvent::new("c", json!({}))).await.unwrap();
...
async fn read_from_gap_at_start_returns_empty() {
    ...
    let events = store.read_from(1, 100).await.unwrap();
    assert!(events.is_empty(), "Gap at start should return empty, got {} events", events.len());
}
...
async fn read_tree_mid_chain_returns_only_that_event() {
    ...
    let root = store.append(AppendEvent::new("root", json!({}))).await.unwrap();
    let child = root.append(AppendEvent::new("child", json!({}))).await.unwrap();
    child.append(AppendEvent::new("grandchild", json!({}))).await.unwrap();
    ...
    assert_eq!(subtree.len(), 1, "Mid-chain read_tree should only return the event itself");
...
async fn empty_payload_is_valid() {
    ...
    let handle = store.append(AppendEvent::new("empty", json!({}))).await.unwrap();
...
async fn deeply_nested_payload_roundtrips() {
    ...
    let handle = store.append(AppendEvent::new("deep", deep_payload.clone())).await.unwrap();
    let event = store.read_event(handle.seq()).await.unwrap().unwrap();
...
async fn large_payload_roundtrips() {
    ...
    let handle = store.append(AppendEvent::new("signal_discovered", large_payload.clone())).await.unwrap();
    let event = store.read_event(handle.seq()).await.unwrap().unwrap();
...
async fn multiple_roots_create_independent_trees() {
    ...
    let root_a = store.append(AppendEvent::new("root_a", json!({}))).await.unwrap();
    let root_b = store.append(AppendEvent::new("root_b", json!({}))).await.unwrap();

    root_a.append(AppendEvent::new("child_a", json!({}))).await.unwrap();
    root_b.append(AppendEvent::new("child_b", json!({}))).await.unwrap();
    root_a.append(AppendEvent::new("child_a2", json!({}))).await.unwrap();
    ...
    assert!(tree_a.iter().all(|e| e.event_type.contains("_a") || e.event_type == "root_a"));
    assert!(tree_b.iter().all(|e| e.event_type.contains("_b") || e.event_type == "root_b"));
}
...
async fn read_by_run_nonexistent_run_returns_empty() {
    ...
    store.append(AppendEvent::new("a", json!({})).with_run_id("run-1")).await.unwrap();
...
async fn sequential_seqs_are_monotonically_increasing() {
    ...
    for _ in 0..20 {
        let handle = store.append(AppendEvent::new("event", json!({}))).await.unwrap();
...
async fn read_from_pagination_works() {
    ...
    let page2 = store.read_from(page1.last().unwrap().seq + 1, 3).await.unwrap();
    assert_eq!(page2.len(), 3);
    assert_eq!(page2[0].seq, 4);

    // Page 3: events 7-9
    let page3 = store.read_from(page2.last().unwrap().seq + 1, 3).await.unwrap();
    assert_eq!(page3.len(), 3);
    assert_eq!(page3[0].seq, 7);

    // Page 4: event 10 (partial page)
    let page4 = store.read_from(page3.last().unwrap().seq + 1, 3).await.unwrap();
    assert_eq!(page4.len(), 1);
    assert_eq!(page4[0].seq, 10);

    // Page 5: empty
    let page5 = store.read_from(page4.last().unwrap().seq + 1, 3).await.unwrap();
...
async fn unicode_and_special_chars_in_payload() {
    ...
    let handle = store.append(AppendEvent::new("unicode_test", payload.clone())).await.unwrap();
    let event = store.read_event(handle.seq()).await.unwrap().unwrap();
