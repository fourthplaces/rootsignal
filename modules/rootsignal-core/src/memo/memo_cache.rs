...
impl MemoCache {
    ...
    pub async fn get(function_name: &str, input_hash: &str, pool: &PgPool) -> Result<Option<Self>> {
        ...
        if let Some(ref row) = row {
            ...
            tokio::spawn(async move {
                let _ =
                    sqlx::query("UPDATE memo_cache SET hit_count = hit_count + 1 WHERE id = $1")
                        .bind(id)
                        .execute(&pool)
                        .await;
            });
    ...
    pub async fn evict_expired(pool: &PgPool) -> Result<u64> {
        let result = sqlx::query(
            "DELETE FROM memo_cache WHERE expires_at IS NOT NULL AND expires_at <= now()",
        )
        .execute(pool)
        .await?;
