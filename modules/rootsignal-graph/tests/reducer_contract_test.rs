...
fn malformed_payload_returns_deserialize_error() {
    ...
    assert!(result.is_err(), "Malformed GatheringDiscovered payload should fail deserialization");
}
...
fn build_all_events() -> Vec<Event> {
    ...
    vec![
        ...
        Event::ActorStatsUpdated { actor_id: id, entity_count: 1, last_active: now },
