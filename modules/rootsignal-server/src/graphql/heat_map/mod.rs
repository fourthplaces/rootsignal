...
impl HeatMapQuery {
    async fn heat_map_points(
        ...
        let points = if let Some(zip) = zip_code {
            ...
            rootsignal_domains::heat_map::HeatMapPoint::find_latest_by_type(&et, pool)
                .await
                .unwrap_or_default()
        } else {
            rootsignal_domains::heat_map::HeatMapPoint::find_latest(pool)
                .await
                .unwrap_or_default()
        };
    ...
    async fn signal_gaps(
        ...
        let results = rootsignal_domains::heat_map::HeatMapPoint::signal_gaps(
            signal_domain.as_deref(),
            category.as_deref(),
            limit,
            pool,
    ...
    async fn signal_trends(
        ...
        let results = rootsignal_domains::heat_map::HeatMapPoint::temporal_comparison(
            current_start,
            current_end,
            previous_start,
            previous_end,
