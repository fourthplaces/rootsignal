...
impl ListingWithDistance {
    ...
    pub async fn find_near_zip(
        ...
        qb.push_bind(lat_delta);
        // Compute lng_delta dynamically from the center's actual latitude
...
impl ListingStats {
    pub async fn compute(pool: &PgPool) -> Result<Self> {
        ...
        let (
            ...
            Self::count_by_tag_kind("listing_type", pool),
            Self::count_by_tag_kind("audience_role", pool),
            Self::count_by_tag_kind("category", pool),
            Self::count_by_tag_kind("signal_domain", pool),
            Self::count_by_tag_kind("urgency", pool),
            Self::count_by_tag_kind("confidence", pool),
            Self::count_by_tag_kind("capacity_status", pool),
        )?;
