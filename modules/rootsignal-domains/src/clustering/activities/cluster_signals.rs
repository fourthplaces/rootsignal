...
pub async fn cluster_signals(deps: &ServerDeps, cluster_type: &str) -> Result<ClusterStats> {
    ...
    let similarity_threshold = clustering.similarity_threshold;
    let match_score_threshold = clustering.match_score_threshold;
    let _merge_threshold = clustering.merge_coherence_threshold;
    let geo_radius = clustering.geo_radius_meters;
    let time_window = clustering.time_window_hours;
    let batch_size = clustering.batch_size;
    let ef_search = clustering.hnsw_ef_search;

    // Wrap in transaction so SET LOCAL persists across queries
    ...
    for &item_id in &unclustered_ids {
        ...
        // Get the item's embedding from pre-fetched map
        let embedding = match embedding_map.get(&item_id) {
            Some(e) => e.clone(),
            None => continue,
        };
